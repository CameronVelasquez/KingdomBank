...
public class HomebankingApplication {
	...
	@Bean
	public CommandLineRunner initData(CardServices cardServices, ClientRepository clientRepository, AccountRepository accountRepository, TransactionRepository transactionRepository, LoanRepository loanRepository, ClientLoanRepository clientLoanRepository, CardRepository cardRepository) {
		return (args) -> {
			...
			Account account1 = new Account("VIN001", now, 5000.00, true, AccountType.SAVINGS);
			Account account2 = new Account( "VIN002", nextDay, 7500.00, true, AccountType.CHECKING);
			Account account3 = new Account( "VIN003", nextDay, 7500.00, true, AccountType.CHECKING);
			...
			//Loans
			Loan mortgage = new Loan( "Mortgage", 500000.00, List.of(12,24,36,48,60), 1.10, List.of(1.0,1.05,1.1,1.15,1.2), true);
			Loan personal = new Loan("Personal", 100000.00, List.of(6,12,24), 1.2, List.of(1.0, 1.05, 1.1), true);
			Loan automotive = new Loan("Automotive", 300000.00, List.of(6,12,24,36), 1.15, List.of(1.0,1.05,1.1,1.15), true);
			...
			Card card2 = new Card(CardType.CREDIT, CardColor.TITANIUM, Melba, cvv(), randomNumberCard(cardServices), LocalDate.now(), LocalDate.now().plusYears(5),true );
			Card cardSample = new Card(CardType.CREDIT, CardColor.SILVER, Melba, cvv(), randomNumberCard(cardServices), LocalDate.now().minusYears(5), LocalDate.now().minusDays(1), true);

			//Cards Client2
			Card card3 = new Card(CardType.CREDIT, CardColor.SILVER, Cameron, cvv(), randomNumberCard(cardServices), LocalDate.now(), LocalDate.now().plusYears(5), true);
